...
their documented external prerequisites and report them when that capability is used.
This implemented contract separates what users install from the release checks
maintainers must satisfy. See [release design](../design-docs/standalone-distribution.md)
for packaging responsibilities.

## Archive layout and release identity
...
checksums and a release manifest accompany the six archives. The Git tag is the sole release-version authority. All four release conditions must hold:

| Condition | Required value |
| --- | --- |
| Source commit | `HEAD` equals the tag commit |
| Working tree | Clean, as defined below |
| Tag | Exactly `v<semver>`, restricted to the stable format below |
| Requested version | Tag without its leading `v` |

Development binaries report `devel` honestly.
...
### Accepted tags and source cleanliness
...
### Build, inspect and compare a release
...
### Tag inputs and reproducibility
...
The [completed distribution plan](../exec-plans/completed/standalone-distribution.md)
and [release finalization plan](../exec-plans/completed/standalone-release-finalization.md)
record artifact validation and native execution evidence. They do not imply signing
or platform validation beyond what was actually run.

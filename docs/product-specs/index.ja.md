---
status: active
owner: maintainers
last_verified: 2026-09-08
translation_of: docs/product-specs/index.md
source_sha256: 61c4daa905e868e2fb8d447a545a92858f79584e43f91eb96440e365bcdb1c56
---

[English（翻訳元）](index.md)

# 製品仕様

- [agent-env-mvp](agent-env-mvp.ja.md) · [English](agent-env-mvp.md)
- [cli-contract](cli-contract.ja.md) · [English](cli-contract.md)
- [manifest-v1](manifest-v1.ja.md) · [English](manifest-v1.md)
- [android-emulator](android-emulator.ja.md) · [English](android-emulator.md)

...
`AGENT_ENV_OWNER`を優先します。同じ operation IDを別プロセスから再試行してもリクエストの
識別情報を維持します。ワーカーの`--max-leases`は1以上、ローカルポリシーの上限8以下とします。

ワーカーの`--android-slots`は0〜65を受け付けます。これはローカルの割り当て処理の偶数コンソールポート
5554〜5682に対応します。範囲外の値はTLS初期化・状態作成・登録の前に拒否します。
0はAndroid容量なしとして広告します。

コントローラーはcreate受理時刻に基づく期限を永続化します（既定4時間、最大24時間）。renewが
成功すると、そのrenewの受理時刻と要求TTLから期限を設定します。再送や再起動によって期限を
さらに延ばしません。期限切れでは、実行中の操作がなければ通常の非force destroyを1件予約します。
オフラインのワーカー、不確実な結果、クリーンアップ失敗では、不在を証明するまで予約容量を保持します。
解放済みリースでもqueued/dispatched操作が残るホストは削除できず、削除したホストへの新規操作は拒否します。
...
従います。メタデータ、接続開始、TLS/headerの上限は維持します。4 MiB上限のマニフェストはcreate要求に
1回だけ含めます。省略形式のパッケージはワーカーの検証前に補完し、従来形式では重複マニフェストの
一致検証を維持します。

リモートワーカーのリース応答では、重複するマニフェストとプロセスのcommand/env宣言を省略します。
...
非永続の入力経路ができるまで、リモート Android UIとBrowserの`set-text`は非対応とします。
直接プロトコルへ送信した場合も含め、CLI・コントローラー・ワーカーはjournalへの保存前にテキスト入力を
拒否します。ローカルの`set-text`は引き続き利用できます。

### 操作結果と証拠取得の分離

読み取り専用logs/artifactの結果は、復旧が不確実でもリースの状態を変更しません。
成功したreconcileでは、既に解放済みのローカルリースを確認できます。Compose表示ログの取得量と
保存済みログの合計に上限を設け、超過時はワーカーのメモリーを無制限に使わず、出力が不完全なことを
明示するエラーを返します。検証済みの保持ソースパッケージはCAS展開前に再利用します。
変更済みソースのforce クリーンアップでは、所有するワークツリーを削除する前に上限付きpatchを記録します。

確定した操作の結果と、自動的な証拠公開の結果は別に扱います。リモート応答は`evidence_status`を返し、
公開できない場合は上限付きの`evidence_error`を付けます。証拠の公開準備の失敗によって、成功したtest/UI/
ブラウザー操作を失敗した変更操作へ変えません。公開経路を復旧したら別の成果物操作で証拠を取得し、
証拠の再取得だけを理由に元の操作を繰り返しません。中断されたUI復旧では、実行中のrunと不確実性を保持します。

### ワーカーの再起動と登録拒否

恒久的な登録拒否ではワーカーを終了し、
一時的な通信障害、レート制限、サーバー障害は再試行します。

前の登録がオンラインの間は、別のワーカー incarnationへの交代を認めません。通常の再起動でも前の
heartbeatが期限切れになるまで待ちます。この登録拒否は再試行できます。前のincarnationへ配信済みの
操作は新しいincarnationへpollで再配信しません。元の永続journalを持つワーカーは復旧・結果報告が
できるが、journalを失った場合は予約容量を保持したまま明示的な復旧が必要となります。
処理中のjournalと認証情報の両方を悪意をもって複製した場合を見分ける保証ではありません。

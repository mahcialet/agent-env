...
The “Current verification” entry names the tests, document checks or execution records used to check the behavior described in each finding. The following codes distinguish their scope. Listing a test does not establish that reintroducing the historical defect made it fail.
...
- F: inspected verification test invokes the public package/app/store entry point with real local persistence/filesystem or injected effects. This proves the stated slice, not a whole CLI/provider execution.
...
- H: helper-only verification test; it still checks the local invariant but cannot prove composition/call ordering at the full public entry point.
...
- U: the mapping to a test detecting the original defect is unresolved; nearby coverage is not counted as proof.
...
- Current verification: `app/process_lifecycle_test.go:TestProcessExitDuringSuccessfulProbeCannotBecomeReady`; coverage F.
...
- Current verification: `app/process_lifecycle_test.go:TestProcessKnownNoSpawnCompensatesPreparedState; runtime/process/process_test.go:TestProvenNoSpawnFailureCanReleaseWithoutQuarantine`; coverage F.
...
- Current verification: `cli/process_native_test.go:TestPersistentProcessNativeCLI`; coverage N.
...
- Current verification: `app/plan_process_test.go:TestProcessEndpointAliasWinsOverRawRuntimePort`; coverage F.
...
- Current verification: `app/plan_process_test.go:TestProcessReadinessRejectsLiteralInheritedSecretBeforeSnapshot`; coverage H.
...
- Current verification: `execx/managed_windows_test.go:TestManagedWindowsCompletedJobIgnoresReusedHistoricalPID`; coverage N.
...
- Current verification: `store/sqlite/process_test.go:TestProcessReleasedReservationsCannotResurrect; TestProcessTerminalReservationKeepsPostReleaseQuarantineVisible`; coverage F.
...
- Current verification: `runtime/process/process_test.go:TestLogsRedactOriginalSecretsWhenEnvironmentChanges; TestReceiptFailureStillHasDurableLogRedactionForCompensation`; coverage F.
...
- Current verification: `store/sqlite/process_test.go:TestProcessDynamicReservationAvoidsExternallyBoundPort; TestProcessConcurrentReservationPortsAreDisjoint`; coverage F.
...
- Current verification: `app/process_lifecycle_test.go:TestProcessDestroyPreviewReportsCleanupWithoutEffects`; coverage F.
...
- Current verification: `repoctl docs-check plus recorded final native CI`; coverage D.
...
- Current verification: `browser/cdp/snapshot_test.go:TestFrameClassificationUsesSecurityOrigin; TestOutOfProcessFrameCannotBeSilentlyOmitted; cli/browser_native_test.go`; coverage N.
...
- Current verification: `app/browser_test.go:TestBrowserURLWaitRequiresSubstring; cli/browser_test.go:TestBrowserRejectsMissingOrUnsafeCLIInputBeforeStore`; coverage F.
...
- Current verification: `browser/cdp/capture_test.go:TestNetworkCaptureBoundsEveryPersistedString; TestNetworkCaptureCountsAllStringsAgainstTotalBudget`; coverage C.
...
- Current verification: `browser/cdp/snapshot_test.go:TestDOMNameTruncationIsReported`; coverage C.
...
- Current verification: `paired architecture inspection; docs-check`; coverage D.
...
- Current verification: `browser/cdp/snapshot_test.go:TestGoneCannotSucceedWithTruncatedSnapshot`; coverage C.
...
- Current verification: `browser/cdp/transport_test.go:TestTransportIgnoresUnsubscribedEvents; TestTransportCaptureIgnoresOtherSessionsAndMethods; TestTransportSubscribedOverflowFailsClosed`; coverage C.
...
- Current verification: `app/browser_test.go:TestBrowserSemanticProvenanceSurvivesUncertainInput`; coverage F.
...
- Current verification: `browser/cdp/snapshot_test.go:TestSnapshotDiscardsAXWhenFrameProofChanges; TestWaitRetriesFrameChangesButNeverPublishesPartialEvidence`; coverage C.
...
- Current verification: `runtime/process/cleanup_windows_test.go:TestWindowsStateRemovalRetriesHeldFile; cleanup_test.go:TestStateRemovalRevalidatesOwnerBeforeRetry`; coverage N.
...
- Current verification: `browser/cdp/input_review_test.go:TestPostInsertErrorRemainsUncertain; app/browser_test.go:TestBrowserUncertainMutationRetainsCleanupBarrier`; coverage C+F.
...
- Current verification: `browser/cdp/input_review_test.go:TestInputFocusRedirectRefusesKeyboardDispatch; TestSelectionFocusChangeRefusesTextInsertion; TestInactiveDocumentRefusesKeyboardDispatch; TestActivationMutationRefusesKeyboardDispatch; cli/browser_native_test.go`; coverage N.
...
- Current verification: `browser/cdp/snapshot_test.go:TestDOMSnapshotOriginAndDocumentProof`; coverage C.
...
- Current verification: `browser/cdp/snapshot_test.go:TestUnparentedIframeTargetsArePageScoped; cli/browser_native_test.go`; coverage N.
...
- Current verification: `app/browser_review_test.go:TestBrowserCaptureBoundsAfterRedaction`; coverage F.
...
- Current verification: `browser/cdp/input_review_test.go:TestURLWaitUsesRawURLButPersistsScrubbedEvidence; cli/browser_native_test.go`; coverage N.
...
- Current verification: `browser/cdp/capture_test.go:TestCaptureDeadlineMarksPendingEventsTruncated`; coverage H.
...
- Current verification: `app/browser_review_test.go:TestBrowserRejectsChangedStoredManifest`; coverage H.
...
- Current verification: `app/browser_semantic_bounds_test.go:TestBrowserSemanticBoundsAfterRedaction`; coverage F.
...
- Current verification: `cli/browser_native_test.go:TestBrowserNativeCLI /closed-shadow`; coverage N.
...
- Current verification: `browser/cdp/capture_test.go:TestCaptureDurationIncludesDomainEnable`; coverage C.
...
- Current verification: `browser/cdp/snapshot_test.go:TestSnapshotNodeLimitMarksOnlyOmittedNodes`; coverage C.
...
- Current verification: `browser/cdp/capture_test.go:TestConsoleMarksOmittedArgumentsTruncated`; coverage C.

### HB25 — BI

The test requiring refusal after target mutation passed because the missing isolated context caused failure before the mutation. The repaired test requires the mutation to be reached, as well as an error and no input effects.

- Detected/earliest: S8/S2; escape: `ORACLE_COUPLING,NEGATIVE_FIXTURE_GAP`.
- Production: `browser/cdp/actions.go:act`.
- Current verification: `browser/cdp/actions_test.go:TestNodeChangesDuringOwnershipVerificationNeverInputs`; coverage C.
...
- Current verification: `app/browser_test.go:TestBrowserLockLossDoesNotExposeObservation`; coverage F.
...
- Current verification: `app/browser_test.go:TestBrowserPriorTextRedactionPreservesAuthority; cli/browser_native_test.go`; coverage N.
...
- Current verification: `browser/cdp/actions_test.go:TestSelectAllUsesExplicitEditingCommandOnEveryPlatform; cli/browser_native_test.go`; coverage N.
...
- Current verification: `cli/browser_native_test.go:TestBrowserEvidenceSecretDetection`; coverage H.
...
- Current verification: `cli/browser_native_test.go:TestBrowserNativeCLI startup/sandbox checks`; coverage N.
...
- Current verification: `app/browser_test.go:TestBrowserLifecycleGuards`; coverage F.
...
- Current verification: `tools/repoctl/main_test.go:TestFormattingCRLFAndActualDrift`; coverage H.
...
- Current verification: `source/gitcli/missing_test.go:TestMissingWorktreeThroughParentAlias; TestMissingWorktreeRegistrationIsObservedAndRemoved`; coverage F.
...
- Current verification: `paths/paths_test.go:TestSourceConfinement`; coverage H.
...
- Current verification: `runtime/compose/compose_test.go:TestRenderSelectedClosureAndPolicy; cli/integration_test.go:TestIntegrationConcurrentLeasesClosureAndEvidence`; coverage F.
...
- Current verification: `policy/escape_test.go:TestBindSymlinkAndVolumeDriverCannotEscapePolicy`; coverage H.
...
- Current verification: `app/lifecycle_test.go:TestLifecycleReconcileMissingAndReleasedLeftovers (nearby, not exact original mapping)`; coverage U.
...
- Current verification: `app/lifecycle_test.go:TestLifecycleRejectsSecretSnapshots; app/manifest_credentials_test.go:TestInheritedCredentialDoesNotMatchManifestSchema; evidence/structured_test.go`; coverage F.
...
- Current verification: `cli/diagnostics_test.go; cli/source_origin_test.go:TestPlanManifestOriginIndependentOfRuntimeRef; cli/logs_test.go:TestArchivedComponentLogsThroughCLI; TestLegacyAggregateCannotPretendComponentIsolation`; coverage F.
...
- Current verification: `execx/process_tree_test.go:TestRunnerReapsOrdinaryDescendants; app/cancellation_test.go:TestDestroyCancelsActualCommandTreeBeforeSourceCleanup`; coverage N.
...
- Current verification: `app/cancellation_test.go:TestDestroyRetainsSourceWhenCancellationFinalizationFails`; coverage F.
...
- Current verification: `app/cancellation_test.go:TestDestroyRetainsSourceWhenCancellationFinalizationFails; execx/runner_test.go:TestOutputWriteFailureIsIncompleteEvidence`; coverage F.
...
- Current verification: `store/sqlite/initialization_test.go:TestConcurrentColdOpen; TestConcurrentColdOpenProcesses; TestInitializationRetriesOnlyBoundedBusyContention; TestInitializationDoesNotRetryInvariantFailure`; coverage F.
...
- Current verification: `app/commands_test.go:TestNamedCommandPersistsRedactedEvidence; app/state_root_test.go:TestNamedCommandEvidenceStaysUnderStateRoot`; coverage F.
...
- Current verification: `store/sqlite/store_test.go:TestOperationLockAcrossConnectionsAndExpiry`; coverage F.
...
- Current verification: `execx/process_unix_test.go:TestTerminateProcessGroupWaitsForZombieReaping; TestTerminateProcessGroupPreservesFailures; app/cancellation_test.go`; coverage N.
...
| CONCURRENCY_GAP | One connection/one sequential process hid cold initialization, queue completion, renewal and identity-observation windows. | Existing separate SQLite connections/processes, deterministic mutation points and native tests that verify refusal on identity mismatch. Broader cross-runtime race replay remains pending. Expected S3–S5. |
| HELPER_ONLY | Correct inspection/validation helper was assumed to guarantee following formatting, persistence or routing; public caller was not the oracle. | HP10 now uses Destroy. HP05 tests rejection of readiness secret literals and HB19 tests rejection of changed stored manifests only through helpers. Add a public-call injection assertion if disposition accepts that coverage work. |
...
1. HP05 invokes `validateProcessSecrets` directly. `BuildPlan` currently calls it
   ...
   readiness-literal injection. A public plan/create test requiring rejection of the same readiness literal could

...
| D | [Distribution](../../exec-plans/completed/standalone-distribution.md) | Tests rechecking existing behavior when work resumed, and native Windows failures. |
...
locations of tests checking each row’s required behavior; unless stated otherwise they are also the
tests recorded when the historical issue was fixed. `Yes/provider`,
...
means the complete historical invariant lacks a test that directly demonstrates the full required behavior.
...
| ID / source | Invariant, historical defect and impact | Current implementation → verification test and scope |
| --- | --- | --- |
...
| P13 | S8→S3; HELPER_ONLY, COMPOSITION_GAP | The original test of Doctor behavior passed while subsequent Inventory invoked a different dependency. Existing Doctor→InventoryFor test catches at S3. This explicitly recurs in releaseVersion→release-verify (R15/R18); preventive pattern is paired public-entry tests, not additional helper-only assertions. |
...
| HCR-R05 / F audit | Archive close errors must propagate so failed output cannot be accepted. Earlier code ignored close errors. | `release_archive.go:writeArchive` joins deferred file/ZIP/gzip/TAR close errors; **No test directly injecting a close error and checking its return was located**. Successful round-trip does not prove this historical failure mode. Guard status: **deferred** failure-injection coverage; no current failure asserted. |
...
| R05 | S7→S2; FAILURE_INJECTION_GAP | Close errors were a local error-return contract, but success-only archive tests missed them. Existing error joins are a production guard, not evidence from a test that injected the failure. A failing writer/close fixture is **deferred**; avoid claiming round-trip catches dropped error propagation. |
...
| R16 | S8→S3; COMPOSITION_GAP | Previous build evidence used outside-tree/ignored destinations; the documented generic output path required a non-ignored case. The existing test building to an actual nested output directory catches S3; no cleanliness exception was added. |

...
| --- | --- | --- |
| Manifest and plan | Strict decode, portable relative paths, duration and argv validity, canonical digest, selection of stack roots and all their direct and indirect dependencies, provider snapshots; `config.Parse/Validate`, `app.BuildPlan` | Existing tests rejecting invalid config/application/browser/process settings; all config tests included in baseline race. Unknown fields, aliases, case-colliding runtime names, source traversal, duplicate APK outputs rejected. No new defect confirmed. |
...
| Documentation | Visible rendered anchors, bilingual metadata and indexes, historical scope exclusions | Tests rechecking historical documentation repairs through the full docsCheck entry point; AUDIT-DOCS-001 in [documentation audit](documentation.md). Broader portability/native claims checked against CI evidence; candidate native runs remain pending. |
...
- Repair and behavior tests / final verification: pending Phase B/C. Cover both unsafe sentinels, secret-safe errors, no retries, source retention and later destroy; retain successful/ordinary-failure readiness behavior.
...
- Preventive control: shared safety classification and table-driven full-create tests injecting unconfirmed termination or incomplete output, plus persistent command cleanup gating; future detection S3/S4 before S6. Pending implementation; no global policy changes.

## Phase C resolution and independent review

AUDIT-CLEANUP-001 is ACCEPT and repaired in `readinessCommand`/`runProbe`. Each actual command attempt writes a running run before execution and finishes only after confirmed termination/output, artifact registration and a terminal run write. Safety sentinels survive sanitized diagnostics; unknown command/output leaves the row running and Create quarantined. Forced Destroy and GC honor the existing durable run barrier. Ordinary completed failure can retry and eventual success leaves failed+passed terminal records.

Permanent `readiness_safety_test.go` cases cover both unsafe errors, redacted diagnostics, exactly1attempt, retained sources/running row, later actual forced Destroy, and ordinary failure/success/release. Tests requiring retries and cleanup to stop on unconfirmed termination or incomplete output failed against the pre-repair implementation. Independent review found the first repair still retried a durably canceled run: full store RequestRunCancel reproduction failed0.119s. Preserving context.Canceled and stopping retry fixed the same reproduction (0.110s); it is retained as `TestReviewReadinessCancellationDoesNotRetry`. Reviewer traced GC/Destroy under-lock gates and repeated focused race3times (2.559s). This intermediate failed approach is not hidden behind final green tests. Final candidate validation remains separate.

...
source_sha256: 7e68c7c4c05714896ac556779c19b24c11849661e78362e1239c536b37814235
---
...
以下はすべて現在も適用される。検出段階は S8、最も早く現実的に防げた段階は S2。言語方針には既に要件が明記されていた。現行実装は `tools/repoctl/main.go` と `tools/repoctl/translations.go`。特記しないテストは `tools/repoctl/translation_review_test.go` にあり、補助関数だけでなく `docsCheck` 全体を通る。下表の不正文書の拒否・有効文書の受理を確認するテストは、race 検査付きで成功した（2.296秒）。

| ID / 元の不具合 | 修正した動作を確認するテスト | 以前の検出機会と見逃し | 防止策 / 再発 |
| --- | --- | --- | --- |
| HIST-DOC-01 / 英語版への表示リンクがなくても翻訳メタデータだけで通過 | `TestTranslationRequiresVisibleSourceLink` | S2 にリンク欠落を拒否するテストがなく、正常系 fixture も同じ欠落を含んだ。ORACLE_COUPLING、NEGATIVE_FIXTURE_GAP。S6 は不正文書を受け入れ、S7 は要件別に照合しなかった。 | 非表示・空・別文書・エスケープのリンクを個別変更する既存テスト。フラグメントの再発は下記。 |
...
| HIST-DOC-03 / 新しい完了 Plan を翻訳例外に追加可能 | `TestCompletedPlanExceptionCannotExpandMigrationSet` | 移行時点という境界があるのに、S2 は新規例外の許可を期待した。ORACLE_COUPLING、NEGATIVE_FIXTURE_GAP。S6 はレジストリを信頼し、S7 は将来追加を見なかった。 | 固定4件の許可リストと新規 Plan を翻訳例外へ追加すると拒否されるテストが存在する。別の例外判定元は確認されなかった。 |
...
| HIST-DOC-05 / 英日両索引から翻訳へのリンクを保証しない | `TestIndexRequiresNavigableJapaneseLinks`、`structure_review_test.go` の `TestReviewJapaneseDocumentsRequireBothLocalIndexes` | S2 のリンク欠落を検証するテストは重複リンクの片方だけを消したため、要件を満たしたままだった。ORACLE_COUPLING。S6、S7 は誤解を招く成功結果に依存した。 | 全体呼出しで、リンクの独立削除や非表示リンク・未使用の参照定義への置換を行い、索引要件を満たさない文書を拒否するテスト。索引欠落の現行再発は未確認。 |
...
| HIST-DOC-07 / コメント除去順だけの修正でコード内コメントやコメント内フェンスを誤認 | `TestTranslationRequiresVisibleSourceLink` のコード・コメントを交互に置く正常例 | S2 は表示リンクを欠く文書の拒否に加えて入れ子構文の正常例を必要とした。COMPOSITION_GAP。S7 は各除去処理を見て、合成した解析状態を見なかった。 | 状態を管理する既存解析と正常例は現在も成功。追加の順序不具合は未確認。 |
...
- 既存検証: 表示リンクや本文中の必須見出しを欠く文書を拒否するテストはあるが、リンク先見出しは生の行を別途走査している。
...
- 修正を確認するテスト・解決・最終検証: 採否と Phase C 待ち。
...
- 再発防止案: 表示見出しの共通抽出と全体呼出しで非表示の見出しへのフラグメントリンクを拒否するテスト。今後の検出は S2/S6 を想定。Phase A では未実装。

## Phase C の解決と独立レビュー

AUDIT-DOCS-001 は ACCEPT として修正した。リンク先見出しも `documentProse` を通し、実見出しの重複アンカーを保つ。`TestFragmentsRequireRenderedTargetHeadings` はコード・コメント・インデント内の偽見出しを拒否し、実見出しと実際の重複を順に追加して許可を確認する。修正前の実装はフェンス・コメント内の偽見出しへのリンクを受理したため、拒否を要求するテストが失敗した。インデント内の偽見出しへのリンクは既に拒否できており、その検査も維持した。対象 race は1.071秒で成功。別担当が不正リンクの拒否と有効リンクの受理を確認するテストを race 付き3回実行（1.215秒）し、残る問題はなかった。候補版の全 harness は別途記録する。

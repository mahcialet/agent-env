...
source_sha256: 4b92f16658612235664c93751a359f8d5ee0aeee8690c6e0dcaa736c12c5c987
---
...
元の検出はS8（merge後に投稿された独立PRレビュー）、監査再現はS9。最早防止段階S3（CLI結果表示のpackage integration）。作用とtable表示を組み合わせた検証が不足していた。分類COMPOSITION_GAP/NEGATIVE_FIXTURE_GAP/ORACLE_COUPLING: adapterから表示への結果の不一致、table負例/正常例の不足、JSONだけに基づく期待値。コマンドやschemaの変更は不要。

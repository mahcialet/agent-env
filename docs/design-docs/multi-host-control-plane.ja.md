...
この設計では、全体の配置管理と worker ごとの所有権を分け、操作記録の永続化と
検証済みデータの転送を通して連携する仕組みを説明します。
必須の挙動は[製品仕様](../product-specs/multi-host-control-plane.ja.md)で定めます。
...
incarnation は、稼働する worker の接続世代を表す識別情報です。
...
検証の限界とパスの要件は[移植性の方針](../PORTABILITY.ja.md)を参照してください。
...
CAS（content-addressed store）は、内容の digest でデータを指定する保存領域です。
CAS key は小文字の SHA-256 digest です。`internal/blobstore` は
`<cas-root>/<digest>/data` にバイト列を保存し、呼び出し側のパスでは保存先を選びません。
非公開の一時ファイルへ順次書き込み、サイズと hash を検査してから原子的に公開します。

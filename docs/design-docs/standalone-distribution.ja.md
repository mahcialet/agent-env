...
source_sha256: 993237d2741f488e84f3f1363f87c7a732fb3afacbbc2f03aa32a043d844f649
---
...
パス除去を指定する。指定された新規出力ディレクトリと同じ親の下に専用一時領域を設け、
6 対象すべてを作成する。完成した一式には、実際のアーカイブ・実行ファイルのバイト列から
...
除外する。出力には引き続き同じ親の下の専用一時領域を使い、既存の出力先を拒否する。

...
アーカイブと前提条件を定義します。ビルド識別情報は `internal/buildinfo`、
汎用immutable bytesは `internal/assets` が扱います。リリース処理はrepoctlに置き、
ローカルとCIでargument array・shellなしの同じ実装を使います。

通常のstate rootは `internal/paths` が所有します。リリース出力は明示された呼び出し
側ディレクトリであり、runtime stateと混同しません。将来のembedded companionは、
bytesを一度記述しdigestでmaterializeし、検証済みの既存内容を再利用できます。
asset coreにAndroid固有のコードは入れません。

## パッケージ生成と検査

### 不変のビルド入力

ビルド入力は、呼び出し元の作業ツリーではなく、解決したコミットの専用 checkout とします。
...
専用cloneには空のGit templateを使います。外部のsmudge/process filterがコンパイラ入力を
書き換え、clean filterがその変更を隠すことを防ぎます。ユーザーのGit設定自体は変更せず、
releaseコマンドはglobal設定のcheckout filterに依存しません。

リリース作成はビルドによる副作用の前に不変な Git 識別情報を検証し、`CGO_ENABLED=0` と
パス除去を指定します。ソース作業ツリー外の専用一時領域で6対象すべてを作成します。
ソースの最終検査後、検証済みbytesを新規出力先と同じ親の下の専用領域へコピーし、
完成した一式をrenameで配置します。これにより、非ignoreのツリー内出力や一時領域と
出力先が異なるfilesystemの場合も、生成ファイルをソース変更と誤認せず扱えます。完成した一式には、実際のアーカイブ・実行ファイルのバイト列から
得た checksum と schema 1 の manifest を含みます。バージョンの根拠は Git のみであり、
ビルドツールは実際の Go runtime バージョンを記録します。リリース CI は Go 1.27.1 に固定します。
決定性の保証範囲は、同じソースとツールチェーンでの比較に限ります。

### 同じバイト列を生成するアーカイブ

Go の tar/zip/gzip writer で、要素の順序、相対パス、mode、所有者、時刻を正規化します。
時刻は tag 対象コミットを使い、gzip metadata にホスト識別情報を含めません。
ZIP の UTC 拡張 timestamp は DOS フィールドで表現できない秒を保持します。
対応するコミット時刻は UTC の 1980-01-01 から 2106-02-07 06:28:15 までとし、
共通のアーカイブ形式で扱えるこの範囲の外は拒否します。
英日 README.txt はホストや現在時刻からではなく、ハーネスから決定的に生成します。
現在は runtime companion を埋め込まないため汎用資産のメタデータは空とし、
外部 UI helper の責務境界を維持します。

### 静的検査とネイティブ smoke

静的検査は Go の `debug/buildinfo` で全対象の実行ファイルを調べ、VCS 識別情報、CGO、
プラットフォーム設定を確認します。さらに、リンカーで与え、実行時のビルド識別情報からも読む
`ReleaseRecord` マーカーを検証します。`-trimpath` を使うと Go は `debug/buildinfo` から
リンカーフラグを省くため、静的比較に必要なバージョン・コミットの値はこのマーカーから取得します。
宣言された digest と実際の値も照合し、安全でない要素や想定外の要素を拒否します。
パス漏えいの検査範囲は、制御するメタデータと既知のリポジトリ・一時パスの接頭辞に限ります。
考えられるすべてのホストパスを検出するものではありません。他プラットフォームのバイナリは実行しません。
ネイティブ smoke は別の操作として、ホストに合う対象を展開し、制限した PATH、
外部の作業ディレクトリ、隔離した状態保存先で行います。workflow は再ビルドせず、
同じ候補バイト列を検証と smoke の条件に通してから公開します。
...
配置用の隣接一時領域はソース検査後にだけ使用し、既存の出力先を拒否します。
配置済みの非ignore出力は、後続のreleaseコマンドでは通常の未追跡変更として数えます。
後続コマンドでもソースをcleanに保つ場合は、ignore対象またはツリー外を出力先にします。
checksum 一覧はアーカイブのファイル名順とします。preview 検証は専用領域の `v0.1.0` tag を使い、
呼び出し元や公開 ref を変更しません。実リリースの繰り返し検証は要求された実際の tag 識別情報を維持します。
CI は tag をシェルへ展開せず、`AGENT_ENV_RELEASE_TAG` と `--tag-env` で渡します。

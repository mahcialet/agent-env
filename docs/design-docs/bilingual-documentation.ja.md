...
まず英語版を仕上げ、事実を確認してから日本語へ訳します。文書ごとの目的を明確にし、読者が
知りたいことを軸に構成してください。現在の要件、理由、検査の仕組み、証拠、既知の制約は
分けて説明します。状態が重要な箇所では、実装済み、作業中、延期、未対応、未決定を明示します。
文章を簡潔にしても、必須・禁止などの規則の強さは変えません。

日本語で維持するのは英語の構成ではなく、意味です。自然に読めるよう、節・段落・文の順序を
変えたり、文を分割・統合したり、箇条書きや表にしたりできます。ただし、要件、条件、例外、
制約、状態、証拠は一つも省きません。コマンド、識別子、プロトコル名も正確に保ちます。
見出しを変えたらリンクを直します。文や段落を英語と一対一にそろえる必要はありません。

意味が一意に伝わる場合、日本語では主語を省略して構いません。定着した日本語の専門用語の方が
明確なら、不必要に英語やカタカナを混ぜず、その用語を使ってください。

両言語とも、主語が曖昧なら明示し、一度に一つの内容を説明します。名詞を長く連ねるより、
何がどうするのかを文で示してください。入口の文書には概要と詳細文書へのリンクを置き、
実装の履歴を丸ごと繰り返しません。読者の案内に必要な短い要約は重複しても構いませんが、
詳細仕様とは別に更新される判断基準を作らないでください。

文書の構成を実質的に変更する場合は、編集担当者とは独立したレビューを、次の三段階で行います。

1. 英語版の目的、案内、文章の明瞭さ、状態の区別を確認します。
2. 日本語版を日本語の文書として読み、自然な順序、用語、分かりやすさを確認します。
   英文との文単位の対応は評価基準にしません。
3. 両言語のレビュー後に、意味の一致を照合します。要件、条件、例外、制約、状態、証拠、
   コマンド、リンクにずれがないかを確認します。

指摘と対応はactive ExecPlanに記録します。これらは内容のレビューであり、更新確認用hashや
構造検査では代替できません。

...
[English（正本）](reconciliation-and-gc.md)
...
既定の policy は、期限切れ後の猶予が 5 分、heartbeat の猶予が 1 分です。Domain の対象判定は両方と保護対象の lifecycle state を確認します。App は preview 時に永続化された `running` コマンド記録を拒否し、apply 前に操作ロック下で再確認します。期限切れや未完了の作業は reconciliation の診断から確認でき、未完了のコマンド行を自動的に完了扱いにはしません。

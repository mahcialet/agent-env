...
- 2026-09-09 checkpoint: Phase A/B are complete (documentation-only commit `56b9c2c`); 15 accepted repairs and tests that detect the same defects are implemented. Aggregate matrix, final independent review and stable-candidate verification remain open. Frozen disk-backed Flutter passed73.54s; UI failed75.11s at the first tap, confirming AUDIT-UI-001. Initial candidate Flutter107.45s/UI146.19s and Linux Browser race10.178s pass; UI will be repeated after the final evidence-bound repair. Interim harness/race/integration runs failed while new mobile fixture sizes/API responses were being corrected. One process-preview deadline failure under concurrent integration load requires isolated recheck.
...
| A8 | Major external effects are audited for intent/effect/identity persistence ordering. | Current-control-plane intent/effect/result review; provider receipt/persistence tests and test of durable readiness attempts. |
...
| A12 | Lock loss/cancellation cannot authorize unintended later effects. | Test preventing readiness retries after typed cancellation and existing lock-loss/command-tree/UI/Browser fencing tests; candidate race passes. |
...
| A14 | Android/Browser stale actions revalidate exact targets and reject ambiguous/truncated evidence. | UI editable positive and secret/stale refusal tests, exact helper build and recovery; test detecting CDP document navigation after predicate evaluation. |
| A15 | `gone`/absence waits do not treat truncated evidence as complete absence. | Existing Android wait tests for partial/truncated observations and Browser tests preventing incomplete AX/frame observations from proving absence were retained; reviewed in mobile/Browser matrices. |
...
| A19 | Release tooling is audited for source identity, deterministic bytes and validated-byte publication. | Clean final f2ec634 private-clone release-verify PASS: six targets twice, eight byte-identical files, Linux smoke; release tests detecting validation omissions PASS. |
| A20 | Durable mechanical claims have implementation/negative tests or corrected prose. | Full docsCheck tests verify rejection of nonexistent fragments and acceptance of valid links; UI/DOM bounds and readiness durable claims corrected in both languages; docs-check PASS. |
...
| A22 | Past review defects are mapped to current tests/entry points and checked for stale or weakened coverage. | 186 material rows map current implementation and verification evidence, including the limits of that evidence; historical annexes distinguish replay from source inspection. |
...
| A24 | Historical “why tests missed it” lessons are incorporated into the current audit method/fixtures where still relevant. | Full entry callbacks, durable run/artifact assertions, independent fixture pairs checking rejection/error behavior and normal behavior and actual encoded boundaries added. |
...
| A30 | Every ACCEPT finding has regression evidence before/with fix. | Linked Phase C annexes retain failing frozen overlays and permanent focused tests for the same defects; readiness ordinary cancellation discovered/repaired in independent review. |
...
3. Frozen reproductions use private temporary overlays/clones; only meaningful permanent tests detecting recurrence enter production history.
...
7. No unsolicited QUALITY policy change. Exact-limit semantics are enforced by the existing contract and focused tests that detect limit violations.
8. No noisy syntax-only limit checker: omission depends on domain eligibility, encoding and skipped records. Matrix controls record the conditions used to judge normal behavior and rejection/error behavior instead.

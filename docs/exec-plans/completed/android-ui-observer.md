...
- [x] 2026-09-08: Defined `AGENTENV-UI-STALE`, `AGENTENV-UI-AMBIGUOUS` and `AGENTENV-UI-UNAVAILABLE` in the bilingual product contract; tests checking backend-status diagnostic mapping passed in the latest full Linux unit run.
...
The app/domain/Android adapter/companion/CLI implementation, recovery and focused tests
for UI-operation refusals and failures, including invalid input or state, are complete
at the Linux unit-test level. `go run
...
- 2026-09-08 second real two-Emulator attempt: `TestRealAndroidUIObserver` ran 127.31s and launched both applications, then rejected the fresh semantic tap as stale. Retained before/after evidence matched except for Android window IDs changing from 8 to 11 and the derived fingerprints. Window IDs are ephemeral across UiAutomation reconnections, so they cannot be durable semantic identity. The helper now uses semantic window metadata and has been rebuilt (third production build); the rerun of the same real-Emulator test was pending at that checkpoint and subsequently passed; see final evidence.
...
| --- | --- | --- |
| U1 | Existing Compose, Android and Flutter manifests/leases work unchanged without using UI commands. | 2026-09-08: `repoctl check` at `867a862` passed unit tests preserving existing behavior; `repoctl test-integration` passed real Docker/Compose. Both real Flutter leases reached application launch in attempt 2; observer end-to-end passed in real run 6. |
...
| U4 | Raw/normalized snapshot evidence is bounded, atomically published and auditable. | 2026-09-08 Linux PASS: snapshot tampering/symlink, protocol bounds, evidence failure and tests of atomic-write safety in `repoctl check`; actual single-Emulator snapshot artifacts retained. |
...
| U8 | A stale or ambiguous node reference fails before input and never falls back to an old coordinate. | 2026-09-08 Linux PASS: `TestUIRejectsSnapshotTamperingAndAmbiguity`, `TestUIBackendRefusalsRetainStableDiagnostics`, test detecting backend-provenance validation omissions; actual older-snapshot smoke refused stale input. |
| U9 | Explicit coordinate tap is available and recorded distinctly from semantic tap. | 2026-09-08 real run 6 PASS: explicit coordinate tap and bounded-viewport swipe exercised; tests rejecting coordinates outside the allowed range also pass. |
...
| U12 | Wait/poll operations are bounded and report timeout without hidden continued effects. | 2026-09-08 real run 6 PASS: bounded missing-predicate wait failed as expected; both normal cleanups passed. Unit tests preserving timeout/recovery/fence behavior also pass. |

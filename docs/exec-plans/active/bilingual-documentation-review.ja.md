...
主要修正後のレビューで、処理順に起因する回帰が見つかった。コードspanより先にコメントを除くとコード中のコメント記号を誤認し、fenceより後にコメントを除くとコメント中のfenceを誤認した。順序の変更だけで対処せず、コメントとコードの状態を明示的に追跡する処理に変更し、両ケースを正常系fixtureとして残した。最初の修正commitは`b94a94d`である。追加修正の検証とpushが終わるまでThreadは未解決のままとする。

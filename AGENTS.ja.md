...
- 想定ブランチと現在の作業はactive ExecPlanを正とする。
...
schemaの正本はrepoctlでmigrationから生成し、手作業で維持しない。
強制できる不変条件はvalidatorとテストが担い、文書はその意図を説明する。
対応するvalidatorと不正fixtureが通るまで、規則が強制されるとは主張しない。

## ドキュメントの保守

正本のこのファイルは案内に徹し、150行以下、目安80〜120行に保つ。
...
active planをcompletedへ移す前に、すべての受け入れ要件を完了する。
完了済み計画を移す際はリンクを直し、振り返りを残す。

人が読む永続文書は英語と日本語で維持する。
英語の`*.md`を正本とし、日本語訳は対応する`*.ja.md`に置く。
